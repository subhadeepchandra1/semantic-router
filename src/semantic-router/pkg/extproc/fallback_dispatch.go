...
func (r *OpenAIRouter) executeFallbackCandidate(
	...
	if isClientStreamingRequested(ctx) {
		...
		if translated.Response.Model == "" {
			translated.Response.Model = candidateModel
		}
		if publicID := responseObjectPublicID(ctx); publicID != "" {
			translated.Response.ID = publicID
		} else if translated.Response.ID == "" {
			translated.Response.ID = "chatcmpl-" + ctx.RequestID
		}
		streamBody, streamDiags, streamErr := engine.EncodeResponseStream(format, translated.Response, llmprotocol.StreamContext{
			Context:            ctx.TraceContext,
			Source:             format,
			Target:             format,
			Options:            clientStreamOptions(ctx),
			PublicModel:        candidateModel,
			ResponseID:         translated.Response.ID,
	...
	r.scheduleSemanticResponseMemoryStore(ctx, ctx.SemanticResponse)
	...
	immediateResp := r.buildFallbackImmediateResponse(responseBody, contentType, ctx)
